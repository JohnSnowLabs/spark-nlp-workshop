...
def get_charts (log_file, threshold=0.0, img_prec_rec_f1_path=None, img_macro_micro_avg_path=None):
    ...
    if img_macro_micro_avg_path is not None:
        fig.savefig(f"{img_macro_micro_avg_path}")
        plt.close(fig)        
        with open(img_macro_micro_avg_path, 'r') as f:
            images['macro_micro_avg'] = Image.open(img_macro_micro_avg_path)
    ...
    if img_prec_rec_f1_path is not None:
        fig.savefig(img_prec_rec_f1_path)
        plt.close(fig)                
        with open(img_prec_rec_f1_path, 'r') as f:
            images['prec_rec_f1'] = Image.open(img_prec_rec_f1_path)
    
    return images


def loss_plot(log_path, img_loss_path=None):
    images = {}
    ...
    if img_loss_path is None:
        plt.show()
    else:
        fig.savefig(img_loss_path)
        plt.close(fig)
        with open(img_loss_path, 'r') as f:
            images['loss'] = Image.open(img_loss_path) 
...
# get_charts("i2b2_ner_logs_combinedData_e8.txt", path="./figure.png")